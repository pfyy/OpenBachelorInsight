...
${this.isEnum ? `enum` : this.isStruct ? `struct` : this.isInterface ? `interface` : `class`} ${this.type.name}${inherited ? ` : ${inherited.map((_) => _?.type.name).join(`, `)}` : ``}
{
  ...
  // src/inject/reader.ts
  Il2Cpp.perform(
    function() {
      function get_enemy_count_dict(enemy_lst) {
        const enemy_count_dict = /* @__PURE__ */ new Map();
        const cnt = enemy_lst.method("get_Count").invoke();
        for (let i = 0; i < cnt; ++i) {
          const enemy = enemy_lst.method("get_Item").invoke(i);
          const enemy_id = enemy.field("enemyId").value.content;
          const enemy_cnt = enemy.field("count").value;
          enemy_count_dict.set(enemy_id, enemy_cnt);
        }
        return enemy_count_dict;
      }
      function merge_left_right_enemy_count_dict(left_enemy_count_dict, right_enemy_count_dict) {
      ...
      Il2Cpp.domain.assembly("Assembly-CSharp").image.class("Torappu.DataCenter.EnemyDuelDataCenter").method("AddTeamData").implementation = function(teamData) {
        ...
        const left_enemy_count_dict = get_enemy_count_dict(teamData.field("teamLeft").value);
        const right_enemy_count_dict = get_enemy_count_dict(teamData.field("teamRight").value);
